/* gpusolve.cu
 * This file contains the code to solve a sparse linear system using CUDA's 
 * cusolver library. It takes in a sparse matrix in CSC format, converts 
 * it to CSR format, and then uses cusolver to solve the linear system Ax=b. 
 * The solution is returned to the host. This file works in conjunction with
 * creatematrixarm.cpp which generates the sparse matrix and right-hand side
 * vector.
*/
...
double *solveviacuda(const double *values, const long long unsigned int a_nnz,
  ...
  CUDA_CHECK(cudaMemcpy(d_cscVal, h_cscVal, sizeof(double) * nnz,
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(d_csccol_pts, h_csccol_pts, sizeof(int) * (cols + 1),
                        cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(d_cscRowInd, h_cscRowInd, sizeof(int) * nnz,
                        cudaMemcpyHostToDevice));
  ...
  CUSPARSE_CHECK(cusparseCsr2cscEx2_bufferSize(
      ...
      valType, // valtype
      copyValues, idxBase, alg, &h_buffer));

  CUDA_CHECK(cudaDeviceSynchronize());
  // I don't know why cuda cant just allocate this on the buffer in the above
  // function.
  CUDA_CHECK(cudaMalloc((void **)&d_buffer, sizeof(double) * h_buffer));

  // Perform actual conversion
  /*The documentation states that this function will also work to convert
    CSC2CSR but provides no explaination of how. It states that CSR is really
    just the transpose of CSC so this function will work for either one. It
    performs this by just copying over the col and row indices and REARRANGING
    THE ELEMENT VALUES. This is not intuitive. And a lesson learned.
   */
  CUSPARSE_CHECK(cusparseCsr2cscEx2(
      sphandle,
      cols, // Cols and rows are switched here because its csc 2 csr
      rows, nnz,
      d_cscVal, // unlike documentation csc comes first because we are doing
                // CSC2CSR
      d_csccol_pts, d_cscRowInd, d_csrvalues, d_csrRowPtr, d_csrColInd, valType,
      copyValues, idxBase, alg, d_buffer));
  ...
  CUDA_CHECK(
      cudaMemcpy(d_b, bpass, sizeof(double) * cols, cudaMemcpyHostToDevice));

  cudaDeviceSynchronize(); // For timing

  gettimeofday(&t1, 0);

  CUSOLVER_CHECK(cusolverSpDcsrlsvqr(solhandle, cols, nnz, descrA, d_csrvalues,
                                     d_csrRowPtr, d_csrColInd, d_b, tol,
  ...
  CUDA_CHECK(
      cudaMemcpy(xreturn, d_x, sizeof(double) * cols, cudaMemcpyDeviceToHost));
